...
| [`all-recommendations.md`](./skills/all-recommendations.md) | `/all-recommendations [report-path]` | **After a review.** Batch-implements every finding from the most recent review — Critical, High, Medium, and Low — by dispatching one local implementation agent per file in parallel. Each agent validates findings before applying them (~50% false-positive rate), then runs the build to verify. Pairs with any of the `/review-*` skills above. Does not call the multipov MCP server. |

All code-review skills share the same daily review quota (10/day) and work against the same 8-person content-routed panel — the difference is the `custom_instructions` framing passed to the server. You can adapt any of them as starting points for your own custom review lenses. `/all-recommendations` is the odd one out: it runs entirely locally and consumes the output of the others.
