...
| [`multipov-security.md`](./skills/multipov-security.md) | `/multipov-security [diff-range]` | **Security-focused.** Auth, injection, XSS/CSRF, SSRF, secrets, supply chain, rate limiting. Hard-stops on critical findings before processing lower-severity items. Run before any auth change, any public launch, any new AI integration, or any dependency bump. |
| [`multipov-performance.md`](./skills/multipov-performance.md) | `/multipov-performance [diff-range]` | **Performance-focused.** User-perceived speed and runtime cost: latency, cold start, allocation pressure, rendering cost, bundle size, caching, DB queries. Recommends profiling before fixing anything non-trivial. |
| [`multipov-reliability.md`](./skills/multipov-reliability.md) | `/multipov-reliability [diff-range]` | **Reliability-focused.** Uptime, failure modes, retry behavior, logging, alerting, operational debuggability. Every finding names the concrete failure scenario and the signal that would surface it in production. |
| [`multipov-scaling.md`](./skills/multipov-scaling.md) | `/multipov-scaling [diff-range]` | **Scaling-focused.** Throughput, memory pressure, DB connection pooling, cache stampedes, capacity planning, backpressure. Every finding calls out the scale trigger. Pair with `/multipov-performance` for a one-two punch. |
| [`multipov-tests.md`](./skills/multipov-tests.md) | `/multipov-tests [path-or-glob]` | **Test-quality focused.** Pressure-tests your tests — missing assertions, uncovered paths, over-mocking, flaky risks, weak coverage. Does NOT auto-fix (rewriting a test can mask the bug it was catching). |
| [`multipov-visual.md`](./skills/multipov-visual.md) | `/multipov-visual [flags]` | **UI review.** Captures screenshots (iOS simulator, Playwright browser, or PDF) and sends them to GPT-4o/Gemini/Grok for visual consensus on failures, design polish, and warnings. Currently a local CLI — pending MCP migration once the server exposes a visual endpoint. |
| [`all-recommendations.md`](./skills/all-recommendations.md) | `/all-recommendations [report-path]` | **After a review.** Batch-implements every finding from the most recent review — Critical, High, Medium, and Low — by dispatching one local implementation agent per file in parallel. Each agent validates findings before applying them (~50% false-positive rate), then runs the build to verify. Pairs with any of the `/multipov-*` skills above. Does not call the multipov MCP server. |

Each command prints the selected panel (persona names, IDs, categories, and routing rationales) before the progress bar starts — so you see who's reviewing your work while you wait.

All code-review skills share the same daily review quota (10/day) and work against the same 8-person content-routed panel — the difference is the `custom_instructions` framing passed to the server. You can adapt any of them as starting points for your own custom review lenses. `/multipov` (local doc orchestrator), `/multipov-visual` (local screenshot pipeline), and `/all-recommendations` (local implementation loop) are the exceptions — they run entirely locally.
